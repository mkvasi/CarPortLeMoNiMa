...
public class CarportCalculator {
    ...
    public void calculateRoofDimensions(Carport carport) {
        double extraLengthFlatRoof = 0.10;
        double extraLengthPitchedRoof = 0.15;

        if (!carport.getRoof().isPitchedRoof()) {
            double extraLength = extraLengthFlatRoof * 2; // Total count of extra space for roof. For each length it would be added 10 cm (* 2)
            double extraWidth = extraLengthFlatRoof * 2; // Total count of extra space for roof. For each wide it would be added 10 cm (* 2)
